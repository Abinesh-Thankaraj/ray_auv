...
def launch_setup():
    ...
    sl.include('ray_description', 'state_publisher_launch.py',
    ...
    with sl.group(ns=ns):
        ...
        # pressure sensor
        bridges.append(GazeboBridge(f'{ns}/pressure',
                      'pressure', 'sensor_msgs/FluidPressure', GazeboBridge.gz2ros))
        ...
        if sl.arg('sliders'):
            sl.node('slider_publisher', arguments=[sl.find('ray_description', 'manual.yaml')])
